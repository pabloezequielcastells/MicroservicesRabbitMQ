...
namespace MicroRabbit.Infra.Bus
{
    public sealed class RabbitMQBus : IEventBus
    {
            ...
            where TH : IEventHandler<T>
        {
            ...
            {
                new ArgumentException($"El handler { handlerType.Name } ya fue registrado anteriormente por { eventName }", nameof(handlerType));
            }
        ...
        private async Task ProcessEvent(string eventName, string message)
        {
            if (_handlers.ContainsKey(eventName))
            {
                using(var scope = _scopeFactory.CreateScope())
                {
                    ...
                    {
                        // TODO: validate null variables 

                        // var handler = Activator.CreateInstance(subs);
                        var handler = scope.ServiceProvider.GetService(subs);
